...
const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const playerRef = useRef<Player>({
    ...
    maxHp: 100,
    mana: 100,
    maxMana: 100, // Updated Mana to 100
    mask: MaskType.TEU,
  ...
  const initLevel = useCallback(
    (levelIndex: number) => {
      ...
      obstaclesRef.current = levelData.obstacles
        ? JSON.parse(JSON.stringify(levelData.obstacles))
        : [];
    },
    [activeBuffs],
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (gameState === GameState.PLAYING) {
        const keyMap: { [key: string]: MaskType } = {
          Digit1: MaskType.TEU,
          Digit2: MaskType.LAN,
          Digit3: MaskType.EN,
          Digit4: MaskType.DIA,
        ...
        if (keyMap[e.code]) {
          ...
          if (
            player.unlockedMasks.includes(newMask) &&
            player.mask !== newMask
          ) {
            player.mask = newMask;
  ...
  const update = useCallback(
    (time: number) => {
      ...
      if (mouseRef.current.down && now - player.lastAttackTime > 500) {
        ...
        const attackX = player.faceRight
          ? player.x + player.w
          : player.x - range;
        ...
        for (let i = minionsRef.current.length - 1; i >= 0; i--) {
          if (checkCollision(attackRect, minionsRef.current[i])) {
            ...
            spawnParticles(
              minionsRef.current[i].x,
              minionsRef.current[i].y,
              "#fff",
              3,
            );
          }
      ...
      if (
        keysRef.current["KeyZ"] &&
        now - player.lastSkillTime > maskStats.skillCd
      ) {
          ...
          switch (player.mask) {
            case MaskType.TEU:
              projectilesRef.current.push({
                x: player.x + (player.faceRight ? player.w : 0),
                y: player.y + player.h / 2,
                w: 15,
                h: 15,
                vx: player.faceRight ? 12 : -12,
                vy: -2,
                color: "#ffff00",
                faceRight: player.faceRight,
                damage: 10 * skillDmgMult,
                isPlayerOwner: true,
                lifeTime: 1000,
              });
              break;
            case MaskType.LAN:
              for (let i = 0; i < 8; i++) {
                projectilesRef.current.push({
                  x: player.x + (player.faceRight ? player.w : -20),
                  y: player.y + 10 + Math.random() * 20,
                  w: 25,
                  h: 25,
                  vx: (player.faceRight ? 6 : -6) + Math.random() * 4,
                  vy: (Math.random() - 0.5) * 2,
                  color: "#e74c3c",
                  faceRight: player.faceRight,
                  damage: 2.5 * skillDmgMult,
                  isPlayerOwner: true,
                  lifeTime: 500,
                  type: "FIRE",
                });
              ...
              const fireRange = {
                x: player.faceRight ? player.x + player.w : player.x - 150,
                y: player.y,
                w: 150,
                h: player.h,
              };
            ...
            case MaskType.EN:
              ...
              projectilesRef.current.push({
                ...
                color: "transparent",
                faceRight: player.faceRight,
                damage: 15 * skillDmgMult,
                isPlayerOwner: true,
                lifeTime: 100,
              });
              if (checkCollision(enSkillRect, boss))
                damageBoss(15 * skillDmgMult, true);
              break;
            case MaskType.DIA:
              player.isShielded = true;
              setGameMessage("Bật Khiên Ông Địa!");
              setTimeout(() => {
                if (playerRef.current) playerRef.current.isShielded = false;
              }, 3000);
              break;
          }
        }
      ...
      if (boss.hp > 0) {
        if (boss.level === 4) {
          if (boss.hp > 200) boss.phase = 1;
            ...
            if (now - boss.lastOrbSpawnTime > 3000) {
              ...
              const orbTypes: ("ORB_FIRE" | "ORB_THUNDER" | "ORB_GRASS")[] = [
                "ORB_FIRE",
                "ORB_THUNDER",
                "ORB_GRASS",
              ];
        ...
        if (now < boss.stunnedUntil) {
            ...
            for (const p of projectilesRef.current) {
              if (p.isPlayerOwner) {
                const dist = Math.sqrt(
                  Math.pow(p.x - boss.x, 2) + Math.pow(p.y - boss.y, 2),
                );
                if (dist < 150) {
          ...
          if (currentLevel !== 3) {
            ...
            if (boss.y + boss.h > GAME_HEIGHT) {
              boss.y = GAME_HEIGHT - boss.h;
              boss.vy = 0;
              isBossGrounded = true;
            }
          } else {
          ...
          if (now > boss.attackCooldown) {
            ...
            if (boss.state === "SKILL") {
              ...
              if (skillType === 1) {
                // Vine
                projectilesRef.current.push({
                  x: boss.faceRight ? boss.x + boss.w : boss.x - 100,
            ...
            if (boss.state === "ATTACK") {
              const bossAttackRect = {
                x: boss.faceRight ? boss.x + boss.w : boss.x - 60,
                y: boss.y,
                w: 60,
                h: boss.h,
              };
              if (
                checkCollision(bossAttackRect, player) &&
        ...
        if (p.isPlayerOwner) {
          ...
          if (checkCollision(p, player) && now > player.invulnerableUntil) {
            if (!player.isShielded) {
              ...
              if (
                currentLevel === 2 &&
                player.mask === MaskType.LAN &&
                (p.type === "FIRE" || p.type === "ORB_FIRE")
              ) {
  ...
  const drawMocTinhSprite = useCallback(
    (
  ...
  const drawHoaThanSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawLoiDieuSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawNienThuSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawTeuSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawLanSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawEnSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const drawDiaSprite = useCallback(
    (
      ctx: CanvasRenderingContext2D,
      x: number,
      y: number,
  ...
  const draw = useCallback(() => {
    ...
    const cooldownRemaining = Math.max(
      0,
      (player.lastSkillTime + stats.skillCd - now) / 1000,
    );
