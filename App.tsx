...
const App: React.FC = () => {
  ...
  const [playerStats, setPlayerStats] = useState({ 
    hp: 100, maxHp: 100, mana: 100, mask: MaskType.TEU 
  });
  
  // Boss UI State
  const [bossStats, setBossStats] = useState({ 
    hp: 100, maxHp: 100, name: "Boss" 
  });
  ...
  const handleLevelComplete = (levelId: number) => {
      ...
      if (rewardItem) {
          ...
          if (!inventory.some(i => i.id === rewardItem.id) && !activeBuffs.some(i => i.id === rewardItem.id)) {
    ...
    if (gameState === GameState.LEVEL_TRANSITION) {
      ...
      return (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 text-white z-50 pixel-font">
          <h2 className="text-4xl text-green-400 mb-4">HOÀN THÀNH MÀN {currentLevel}</h2>
          <div className="text-center mb-8 space-y-4">
            <p className="text-2xl text-yellow-300">{prevLevelData.rewardMsg}</p>
                    ...
                    <div className="flex justify-center gap-2">
                        {inventory.map(item => (
                            <div key={item.id} className="flex flex-col items-center">
                                ...
                                <span className="text-xs text-yellow-200 pixel-font">{item.name}</span>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    if (gameState === GameState.VICTORY) {
      return (
         <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 text-white z-50">
          ...
          <p className="text-2xl mt-8 text-white drop-shadow-md">Bạn đã đánh bại Niên Thú và mang lại bình yên!</p>
    ...
    <div className="bg-black border-2 border-red-600 rounded-lg p-3 w-80 shadow-[0_0_10px_rgba(220,38,38,0.5)] mb-2 relative">
       ...
       <div className="flex justify-between items-end mb-2">
          <div className="flex gap-2 items-center">
             <span className="text-yellow-400 font-bold pixel-font text-xs uppercase tracking-widest">{label}</span>
             <span className="text-yellow-400 font-mono text-sm font-bold tracking-widest">
               - HP: {Math.ceil(current)}/{Math.ceil(max)}
             </span>
          </div>
          {level && <span className="text-gray-400 text-xs pixel-font">LV {level}</span>}
       </div>
       ...
       {subLabel && <div className="mt-2 text-[10px] text-gray-400 font-mono italic">{subLabel}</div>}
    </div>
  );
      ...
      <div className="absolute top-0 left-0 w-full p-4 pointer-events-none flex justify-between items-start">
        ...
        <div className="flex flex-col pointer-events-auto">
            ...
            <StatBox 
                ...
                subLabel={
                    <div className="text-yellow-200">
                       Mặt nạ: <span style={{ color: maskInfo.color }} className="font-bold uppercase">{maskInfo.name}</span>
                    </div>
                }
            />
            ...
            <div className="bg-black border-2 border-blue-600 rounded-lg p-3 w-80 shadow-[0_0_10px_rgba(37,99,235,0.5)] mb-2 relative">
                <div className="flex justify-between items-end mb-2">
                    <div className="flex gap-2 items-center">
                        <span className="text-cyan-400 font-bold pixel-font text-xs uppercase tracking-widest">MANA</span>
                ...
                <div className="w-full bg-gray-900/80 h-4 rounded border border-gray-700 relative overflow-hidden">
                    <div 
                        ...
                        style={{ width: `${Math.max(0, (playerStats.mana / 100) * 100)}%` }}
                    ></div>
                    <div className="absolute top-0 left-0 w-full h-1/2 bg-white/10 pointer-events-none"></div>
                </div>
            </div>
            ...
            <div className="flex gap-2 mt-2">
                {activeBuffs.map(buff => (
                    <div key={`buff-${buff.id}`} className="w-10 h-10 bg-green-900/80 border border-green-400 rounded flex items-center justify-center text-xl relative group shadow-md" title={buff.desc}>
                        {buff.icon}
